...
public class BoardModel {
	...
	java.awt.Point lastMove;//stored as a convenience to AI; not part of state
	
	byte[][] pieces; //[column][row] 1, 2, or null for empty
	
	private BoardModel(int width, int height, int k, boolean gravity){//new board
		this.width = width;
		this.height = height;
		this.kLength = k;
	...
	//has side effects on p
	BoardModel placePiece(java.awt.Point p, byte player){
		assert (pieces[p.x][p.y] == 0);
		BoardModel nextBoard = (BoardModel) this.clone();
		//while gravity is on and there is space under the piece
		while(gravity && p.y > 0 && pieces[p.x][p.y-1] == 0)
			--p.y;//drop the piece
		nextBoard.lastMove = p;
		nextBoard.pieces[p.x][p.y] = player;
		nextBoard.spacesLeft = spacesLeft - 1;
		return nextBoard;
	}
	
	static BoardModel newBoard(int width, int height, int k, boolean gravity){
		return new BoardModel(width, height, k, gravity);
	}
	
	byte getSpace(Point p){
	...
	//two games are equal if their shape, rules, and pieces are equal
	@Override
